...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{FEDORA_APP_UPDATES_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{FEDORA_APP_UPDATES_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require Fedora::App::Updates;

    Fedora::App::Updates->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
fedora_app_updates_server.pl - Catalyst Testserver

=head1 SYNOPSIS

fedora_app_updates_server.pl [options]
